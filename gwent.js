...
class Player {
	...
	async startTurn(){
		...
		if (this.controller instanceof ControllerRemote) {
			await this.controller.startTurn(this);
		}
	}
	...
	passRound(){
		console.log(`Player ${this.id} passing round`);
		if (this === player_me && network.isMultiplayer) {
			network.sendMove({ type: 'pass' });
		}
		this.setPassed(true);
		this.endTurn();
	}
	...
	async playCardToRow(card, row){
		console.log(`playCardToRow: this.id=${this.id}, player_me.id=${player_me ? player_me.id : 'null'}, isMultiplayer=${network.isMultiplayer}`);
		if (this === player_me && network.isMultiplayer) {
			let rowName;
			if (row === weather) {
				rowName = "weather";
	...
	initializeFromID(card_id_list, player){
		...
		if (network.isMultiplayer) {
			if (network.playerIndex === 0) {
				...
				// Map to absolute room index: player_me = network.playerIndex, player_op = 1 - network.playerIndex
				const indices = this.cards.map(c => card_dict.findIndex(cd => cd.name === c.name));
				const playerAbsIndex = (player.id === 0) ? network.playerIndex : 1 - network.playerIndex;
				network.sendMove({ type: 'sync_deck', playerAbsIndex: playerAbsIndex, indices: indices });
				if (player.id === 0) game.meDeckSynced = true;
...
class Game {
	...
	async startGame() {
		...
		if (network.isMultiplayer) {
			// Wait for both decks to be synced before drawing
			while (!this.meDeckSynced || !this.opDeckSynced) await sleep(100);
		}
		
		// Sequential draws so each call gets the correct deck.cards[0].
		// Parallel draws cause a race: all 10 capture the same cards[0] reference,
		// then 9 of them fall back to splice(-1,1) and pull from the wrong end,
		// leaving the two clients with different hands.
		for (let i = 0; i < 10; i++) {
	...
	async coinToss(){
		if (network.isMultiplayer) {
			// In multiplayer, host (playerIndex 0) decides
			if (network.playerIndex === 0) {
				this.firstPlayer = (Math.random() < 0.5) ? player_me : player_op;
				// Send absolute index so guest can correctly map to their player_me/player_op
				const firstPlayerAbsIndex = (this.firstPlayer === player_me) ? network.playerIndex : 1 - network.playerIndex;
				network.sendMove({ type: 'coin_toss', firstPlayerAbsIndex: firstPlayerAbsIndex });
			} else {
				// Wait for coin_toss move
				while (!this.firstPlayer) await sleep(100);
			}
		} else {
			this.firstPlayer = (Math.random() < 0.5) ? player_me : player_op;
		}
	...
	async initialRedraw(){
		if (network.isMultiplayer) {
			...
			while (!this.opponentReady) await sleep(100);
		} else {
			for (let i=0; i< 2; i++)
	...
	async startRound(){
		this.roundCount++;
		this.currPlayer = (this.roundCount%2 === 0) ? this.firstPlayer.opponent() : this.firstPlayer;
...
class UI {
   ...
   pCard.holder.endTurn();
	...
	async selectRow(row){
		...
		if (card.name === "Scorch"){
			this.hidePreview();
			await holder.playScorch(card);
		} else if (card.name === "Decoy") {
			return;
		} else {
			await holder.playCardToRow(card, row);
		}
	}
...
class DeckMaker {
	...
	// Returns the current deck's data for multiplayer
	getDeckData() {
		// Temporarily disabled requirements for multiplayer testing
		/*
		let warning = "";
	...
	startNewGame(){
		if (network.isMultiplayer) return;
		let warning = "";
		if (this.stats.units < 22)
			warning += "Your deck must have at least 22 unit cards. \n";
		if (this.stats.special > 10)
